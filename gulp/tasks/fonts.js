...
export const ttfToWoff = () => {
    ...
    .pipe(app.gulp.dest(app.path.build.fonts)) //выгружаем результат

    .pipe(app.gulp.src(`${app.path.srcFolder}/fonts/*.ttf`))//ищем woff
    .pipe(ttf2woff2()) //конвертируем в woff 2
    .pipe(app.gulp.dest(app.path.build.fonts)) //выгружаем результат
}
    ...
    fs.readdir(app.path.build.fonts, function(err,fontsFiles) { 
        if (fontsFiles) {
            ...
            if (!fs.existsSync(fontsFile)) {
                ...
                console.log('Файл со шрифтами уже существует')
            }
        }
    });
